...
import { createGeneration, serializeGeneration, GenerationQuotaError } from "@/lib/generations";
...
const bodySchema = z.object({
  ...
  webhook_url: z
    .string()
    .url()
    .refine((v) => /^https?:\/\//i.test(v), "webhook_url must use http or https")
    .nullish(),
});
...
export const POST = withApiKey(async (req, user) => {
  ...
  try {
    ...
    if (err instanceof GenerationQuotaError) {
